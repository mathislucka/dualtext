...
class TestCurrentUserStatisticsView(APITestCase):
    def test_current_user_statistics(self):
        ...
        task_1 = TaskFactory(project=project, annotator=user, is_finished=True)
        task_2 = TaskFactory(project=project, annotator=user, is_finished=True)
        task_3 = TaskFactory(project=project, annotator=user, action=Task.REVIEW, copied_from=task_1)
        task_4 = TaskFactory(project=project, annotator=user, action=Task.REVIEW, copied_from=task_1, is_finished=True)
